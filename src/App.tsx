        ...
        <div className="sheet-scroll">
          ...
          {draft.title.trim() && <section className="research-panel">
            ...
            {research ? <div className="research-result">
              ...
              <div className="market-summary"><div><span>Páginas útiles</span><b>{new Set(research.sources.map((source) => source.url)).size || '—'}</b></div><div><span>Precios detectados</span><b>{research.listings.length || '—'}</b></div><div><span>Referencias orientativas</span><b>{research.comparables.filter((listing) => listing.sourceType !== 'guide').length || '—'}</b></div><div><span>Valor principal</span><b>{research.asking.median == null ? '—' : money(research.asking.median)}</b></div><div><span>Ventas cerradas</span><b>{research.sold.available ? `${research.sold.count || 1}${research.sold.median != null ? ` · ${money(research.sold.median)}` : ''}` : 'No verificadas'}</b></div></div>
              {research.comparables.some((listing) => listing.sourceType === 'guide') && <div className="comparable-prices"><h4>Referencia principal · hobbyDB Price Guide</h4>{research.comparables.filter((listing) => listing.sourceType === 'guide').slice(0,1).map((listing) => <a className="comparable-price primary-guide" key={listing.id} href={listing.url} target="_blank" rel="noreferrer"><span><b>{listing.title}</b><small>Estimated Value de hobbyDB · abrir ficha</small></span><strong>{listing.originalPrice != null && listing.originalCurrency ? money(listing.originalPrice, listing.originalCurrency) : money(listing.price, listing.currency)}</strong></a>)}</div>}
