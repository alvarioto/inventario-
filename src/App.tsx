...
import { auth, appCheckConfigured, demoMode, isFirebaseConfigured } from './lib/firebase';
import {
  fileToDataUrl,
  identifyWithAi,
  lookupIsbn,
  ...
  return (
    <section><div className="page-heading"><div><span className="eyebrow">CONFIGURACIÓN</span><h1>Ajustes y copias</h1><p>Tu colección es tuya. Puedes sacarla completa cuando quieras.</p></div></div>
      <div className="settings-grid">
        ...
        <div className="panel"><PanelHeader title="Estado Firebase"/><div className="status-list"><div><span className={isFirebaseConfigured ? 'dot ok':'dot warn'}/><b>Configuración web</b><em>{isFirebaseConfigured ? 'Conectada' : 'Pendiente'}</em></div><div><span className={demoMode ? 'dot warn':'dot ok'}/><b>Base de datos</b><em>{demoMode ? 'Local demo' : 'Cloud Firestore'}</em></div><div><span className={demoMode || !appCheckConfigured ? 'dot warn':'dot ok'}/><b>IA visual</b><em>{demoMode ? 'Demo' : appCheckConfigured ? 'Firebase AI Logic' : 'Falta App Check'}</em></div></div></div>
        <div className="panel"><PanelHeader title="Privacidad"/><p className="muted">Las reglas limitan Firestore al UID autenticado. Las fotos se comprimen y se guardan dentro de Firestore; no usamos Cloud Storage, Cloud Functions ni Secret Manager.</p><div className="secure-note"><ShieldCheck size={17}/> La IA usa Firebase AI Logic con Gemini Developer API; no necesitas meter una clave secreta en la app.</div></div>
      </div>
    </section>
  );
}
      ...
      <form className="item-sheet" onSubmit={submit}>
        ...
        <div className="sheet-scroll">
          ...
          {item && <section className="research-panel">
            ...
            {research && <div className="research-result"><p>{research.summary}</p><div className="market-summary"><div><span>Anuncios comparables</span><b>{research.asking.count || '—'}</b></div><div><span>Mediana solicitada</span><b>{research.asking.median == null ? '—' : money(research.asking.median)}</b></div><div><span>Ventas cerradas</span><b>{research.sold.available ? 'Disponible' : 'No disponible'}</b></div></div><div className="research-facts">{research.facts.slice(0, 8).map((fact) => <div key={`${fact.label}-${fact.sourceId}`}><b>{fact.label}</b><span>{fact.value}</span></div>)}</div><div className="source-list">{research.sources.slice(0, 8).map((source) => <a key={source.id} href={source.url} target="_blank" rel="noreferrer">{source.kind === 'ebay' ? 'eBay' : 'Fuente'} · {source.title}</a>)}</div>{research.warnings.map((warning) => <small className="warning-line" key={warning}>{warning}</small>)}</div>}
          </section>}
