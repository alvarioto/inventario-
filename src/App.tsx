  ...
  return (
    <section className="scan-page">
      ...
      <div className="scan-shortcuts">
        <button type="button" className="scan-shortcut barcode-shortcut" onClick={() => barcodeScanRef.current?.click()} disabled={photoBusy || busy}>
          <Barcode size={24}/><span><b>Leer código de barras</b><small>EAN / UPC / ISBN con la cámara</small></span>
        </button>
        <button type="button" className="scan-shortcut" onClick={() => cameraInputRef.current?.click()} disabled={files.length >= photoLimit || photoBusy || busy}>
          <Camera size={24}/><span><b>Fotografiar artículo</b><small>Identificación por imagen</small></span>
        </button>
      </div>
      <div className="scan-layout">
        ...
        <div className="scan-side">
          ...
          <div className="panel barcode-box"><div className="panel-head"><h2><Barcode size={19}/> Código de barras / ISBN</h2></div><p>Este lector es propio de FrikiVault: usa la cámara del móvil y no depende de Coleka, LegendsVerse ni ninguna web.</p><button type="button" className="secondary wide barcode-panel-button" onClick={() => barcodeScanRef.current?.click()} disabled={photoBusy || busy}><Barcode size={18}/> Leer con cámara</button><div className="inline-field"><input value={barcode} onChange={(e) => setBarcode(e.target.value)} placeholder="EAN / UPC / ISBN"/><button onClick={useBarcode} disabled={busy || !barcode}>Buscar código</button></div></div>
        </div>
      </div>
    </section>
  );
        ...
        <div className="sheet-scroll">
          ...
          {draft.title.trim() && <section className="research-panel">
            ...
            {research ? <div className="research-result">
              ...
              {research.links.coleka && <p className="source-routing-note"><b>Fuentes de figuras:</b> Coleka como catálogo general; LegendsVerse cuando es Marvel Legends; FigureRealm para líneas antiguas y variantes. El precio automático solo se muestra cuando una fuente devuelve una coincidencia suficientemente exacta.</p>}
              <div className="source-list">{research.links.coleka && <a href={research.links.coleka} target="_blank" rel="noreferrer">Coleka · catálogo general</a>}{research.links.legendsVerse && <a href={research.links.legendsVerse} target="_blank" rel="noreferrer">LegendsVerse · Marvel Legends</a>}{research.links.figureRealm && <a href={research.links.figureRealm} target="_blank" rel="noreferrer">FigureRealm · antiguas/variantes</a>}{research.links.actionFigure411 && <a href={research.links.actionFigure411} target="_blank" rel="noreferrer">ActionFigure411 · respaldo</a>}<a href={research.links.ebay} target="_blank" rel="noreferrer">eBay · informativo</a><a href={research.links.sold} target="_blank" rel="noreferrer">eBay vendidos · comprobar</a>{research.links.priceCharting && <a href={research.links.priceCharting} target="_blank" rel="noreferrer">PriceCharting</a>}{research.links.stockx && <a href={research.links.stockx} target="_blank" rel="noreferrer">StockX</a>}{research.sources.filter((source) => ![research.links.legendsVerse,research.links.actionFigure411,research.links.priceCharting].filter(Boolean).includes(source.url)).slice(0,4).map((source) => <a key={source.id} href={source.url} target="_blank" rel="noreferrer">{source.title}</a>)}</div>
