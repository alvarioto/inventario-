...
function candidateFallbackTitle(url){try{return decodeURIComponent(new URL(url).pathname.split('/').filter(Boolean).pop()||'').replace(/[-_]+/g,' ').trim();}catch{return '';}}
function extractCandidates(html){const rows=new Map();const rx=/<a\b[^>]*href=["']([^"']*\/marketplaces\/hobbydb\/catalog_items\/[^"'#?]+(?:\?[^"']*)?)["'][^>]*>([\s\S]*?)<\/a>/gi;let match;while((match=rx.exec(String(html)))){const rawHref=match[1];if(/\/select_type(?:\?|$)/i.test(rawHref))continue;let url;try{url=new URL(rawHref,HOBBYDB).href;}catch{continue;}let title=decodeHtml(match[2]);if(!title)title=candidateFallbackTitle(url);const existing=rows.get(url);if(!existing||(!existing.title&&title))rows.set(url,{url,title});}return [...rows.values()];}
...
function pageVariant(text,title=''){const hay=normalize(`${title} ${extractField(text,'Production Status')} ${extractField(text,'Characteristics')}`);if(/\bchase\b/.test(hay))return 'chase';if(/\bflocked\b/.test(hay))return 'flocked';if(/\bglow in the dark\b|\bgitd\b/.test(hay))return 'glow';if(/\bmetallic\b/.test(hay))return 'metallic';if(/\bdiamond\b/.test(hay))return 'diamond';if(/\bblack light\b/.test(hay))return 'black light';if(/\bchrome\b/.test(hay))return 'chrome';return 'classic';}
function exactMatch(item,html,candidateTitle=''){const text=decodeHtml(html);const title=[extractTitle(html),candidateTitle].filter(Boolean).join(' ');const brand=extractField(text,'Brand');const series=extractField(text,'Series');const ref=extractField(text,'Reference #');const character=normalize(item.character);if(brand&&!/\bfunko\b/i.test(brand))return {ok:false,reason:'brand'};if(series&&!/\bpop!?\b/i.test(series))return {ok:false,reason:'series'};if(character&&!normalize(`${title} ${text.slice(0,900)}`).includes(character))return {ok:false,reason:'name'};if(item.popNumber&&ref&&String(ref).match(/\d{1,5}/)?.[0]!==String(item.popNumber))return {ok:false,reason:'number'};const wanted=requestedVariant(item),actual=pageVariant(text,title);if(wanted==='classic'&&actual!=='classic')return {ok:false,reason:`variant:${actual}`};if(wanted!=='classic'&&wanted!==actual)return {ok:false,reason:`variant:${actual}`};return {ok:true,variant:actual,title,text};}
...
async function resolveItem(item,cookie){
 ...
 if(item.hobbydbUrl){
  ...
  const isItem=/\/marketplaces\/hobbydb\/catalog_items\/[^/?#]+/i.test(url.pathname);
  if(isItem){try{const direct=await inspectCandidate(item,url.href,cookie);if(direct)return direct;}catch{}}
  else if(/\/marketplaces\/hobbydb\/catalog_items\/?$/i.test(url.pathname)){
   try{for(const candidate of await candidatesFromUrl(url.href,cookie)){if(!candidateMap.has(candidate.url))candidateMap.set(candidate.url,candidate);}}catch{}
  }
 }
